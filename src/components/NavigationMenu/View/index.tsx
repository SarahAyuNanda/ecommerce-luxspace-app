  ...
  return (
    <aside className={`fixed top-0 right-0 z-10 md:hidden w-screen h-screen flex justify-between items-start flex-row transition ease-in-out duration-700 ${isOpen ? 'translate-y-0' : '-translate-y-full'}`}>
      <div className='relative w-full h-screen py-10 px-7 bg-white shadow-light'>
        {/* --------------------------------- CLOSE BUTTON ---------------------------------- */}
        <button id='close-menu' className='fixed top-0 right-0 m-5 flex z-50 items-center justify-center w-9 h-9 p-1 rounded-full ease-in-out duration-300 hover:bg-gray-100 active:bg-gray-100' onClick={onClose}>
        ...
        <div className='flex flex-col w-full mt-8'>
          {data.map(item => (
            <button key={item.label} onClick={onClose} className='w-full focus:outline-none hover:bg-tuscany rounded transition-all'>
              <div className='flex justify-center items-center flex-row gap-5 p-3 font-montserrat hover:text-white'>
                {/* {item.icon} */}
                <p>{item.label}</p>
              </div>
            </button>

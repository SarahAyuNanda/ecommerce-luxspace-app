...
const DropdownLanguageView = ({ isShow, value, data, onOpen, onSelect }: IDropdownLanguage) => {
  return (
    <div className='relative p-1 rounded-full ease-in-out duration-300 active:bg-gray-100'>
      ...
      {/* ------------------------------- DROPDOWN LANGUAGE ------------------------------- */}
      <ul className={`${isShow ? 'block' : 'hidden'} absolute flex flex-col z-50 top-10 right-0 w-max bg-white rounded shadow-lg overflow-visible font-montserrat ease-in-out duration-700`} aria-orientation='vertical' role='listbox' tabIndex={0}>
        {data.map((item, idx) => (
